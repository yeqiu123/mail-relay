...
- Create public mailbox links on a separate domain. Visitors complete a server-validated raster CAPTCHA before browsing newest-first message pages.
...
Password resets and account status changes invalidate existing login sessions. Refresh jobs are deduplicated per mailbox and admission-limited per user.
...
Docker Compose starts a web container and a health-checked task worker. The browser does not poll the mailbox list automatically. The worker exclusively rotates tokens, processes durable manual refresh jobs, and synchronizes all unseen IMAP UIDs every `MAIL_SYNC_INTERVAL_SECONDS` (five minutes by default). Each mailbox sync uses one IMAP session and writes bounded batches. `MAIL_FETCH_MAX_BYTES` limits the bytes fetched from one message, while `MAIL_ARCHIVE_MAX_MESSAGES` limits retained messages per mailbox. If IMAP UIDVALIDITY changes, a replacement snapshot is staged before the existing archive is atomically replaced. Shared links read the local archive in newest-first pages. Token rotation defaults to seven days. A refresh token can still be invalidated by revoked consent, password changes, account protection, or Microsoft policy.
